...
import type { LionClawAPI, StreamChunk, ConfirmAction, AskQuestionRequest, AskQuestionResponse, AuditEntry, SystemLogEntry, PipelineStreamChunk, PipelinePhaseChangedEvent, PipelineNotesUpdatedEvent, PipelineSprintCompleteEvent, PipelineProjectUpdatedEvent, ChatAttachment, IngestionProgress, IngestJob, PipelineConversationPhases, PipelinePhaseArtifact, SyncAgentsToOrchestratorRequest, DriveState, CodexWindowsHealthWarning, CodexPatchFailureWarning, CodexWindowsPrepSkipped, PipelineAuditAgentsStateResponse } from '../../src/types';
...
const api: LionClawAPI = {
  ...
  chat: {
    ...
    // UX-CTX (fix 2): contextUsage persistido da sessao (hidrata a barrinha ao
    // abrir/trocar de sessao). null = sem barra (janela desconhecida / sem tokens).
    getContextUsage: (sessionId: string) => ipcRenderer.invoke('chat:get-context-usage', sessionId),
    // SPEC chat-context-reduction A.3 (S2): canais ADITIVOS de toggles por
    // sessao. O `featureToggles` do send viaja dentro de `options` (acima).
    ...
    onCompactionActive: (cb: (payload: { isActive: boolean; modelLabel?: string; source?: 'lionclaw' }) => void) => {
      const handler = (_: Electron.IpcRendererEvent, payload: { isActive: boolean; modelLabel?: string; source?: 'lionclaw' }) => cb(payload);
      ipcRenderer.on('compaction:active', handler);
      return () => { ipcRenderer.removeListener('compaction:active', handler); };
    },
  },
  activity: {
    // Activity Log v2 (SPEC K2 secao 5.7): hidratacao do painel por blocos no
    // load/switch de sessao. Invoke one-shot — sem listener, sem cleanup.
  ...
  mcp: {
    ...
    // SPEC robustez-chat SB-7 (P8, AC-B18): server que falha start/discovery
    // fica visivel — canal aditivo mcp:status-changed { id, status, error? }.
    onStatusChanged: (cb: (payload: { id: string; status: 'running' | 'stopped' | 'error'; error?: string }) => void) => {
      const handler = (_e: unknown, payload: { id: string; status: 'running' | 'stopped' | 'error'; error?: string }) => cb(payload);
  ...
  logs: {
    ...
    streamSystem: (cb: (entry: SystemLogEntry) => void) => {
      ...
      return () => { ipcRenderer.removeListener('logs:system-entry', handler); };
    },
  },
  usage: {
  ...
  vault: {
    ...
    // SPEC robustez-chat SB-9: saude do vault (KEYTAR-DEGRADED / VAULT-CORRUPT / SECRET-UNREADABLE)
  ...
  provider: {
    ...
    // SPEC-001 §15 / S12: orchestrator provider lifecycle.
    listStatuses: () => ipcRenderer.invoke('provider:list-statuses'),
    check: (payload: { runtime: string; provider: string }) =>
      ipcRenderer.invoke('provider:check', payload),
    connect: (payload: {
      provider: string;
      apiKey?: string;
      baseUrl?: string;
    ...
    disconnect: (payload: { provider: string }) =>
      ipcRenderer.invoke('provider:disconnect', payload),
    // SPEC-003 §7.3 / §8: explicit Vertex Gemini connectivity test bridge.
  ...
  pipeline: {
    ...
    // (I1 - SPEC drive-ux-pack) Canal NOVO 'pipeline:messages-updated' (R2:
    // aditivo, sem rename): mensagens persistidas da fase mudaram (reply do
    // drive); o pipeline-store re-busca o historico via loadPhaseHistory.
    onMessagesUpdated: (cb: (data: { projectId: string; phase: number }) => void) => {
      ...
      return () => { ipcRenderer.removeListener('pipeline:messages-updated', handler); };
    },
    ...
    onSprintsLoaded: (cb: (data: { projectId: string; sprints: Array<{ index: number; name: string; status: string; coderAgentId?: string; evaluatorAgentId?: string; sprintJsonId?: string; sprintId?: string }> }) => void) => {
      const handler = (_: Electron.IpcRendererEvent, data: { projectId: string; sprints: Array<{ index: number; name: string; status: string; coderAgentId?: string; evaluatorAgentId?: string; sprintJsonId?: string; sprintId?: string }> }) => cb(data);
    ...
    onAuthRequired: (cb: (data: { projectId: string; phaseNumber: number; agentId: string; message: string; provider: 'codex' | 'grok' | 'kimi'; runtime: 'codex' | 'grok' | 'kimi'; ownerKind?: 'pipeline' | 'harness' | 'enrich'; roundId?: string }) => void) => {
      const handler = (_: Electron.IpcRendererEvent, data: { projectId: string; phaseNumber: number; agentId: string; message: string; provider: 'codex' | 'grok' | 'kimi'; runtime: 'codex' | 'grok' | 'kimi'; ownerKind?: 'pipeline' | 'harness' | 'enrich'; roundId?: string }) => cb(data);
    ...
    resumeAfterAuth: (projectId: string, provider?: 'codex' | 'grok' | 'kimi') =>
      ipcRenderer.invoke('pipeline:resume-after-auth', projectId, provider) as Promise<{ ok: true } | { ok: false; message: string }>,
  },
  dialog: {
  ...
  codex: {
    ...
    // spec-gpt56 S3: capabilities descobertas (model/list) — discovered-first na UI.
  ...
  // SPEC-011 §10: namespace kimi:* (runtime Kimi nativo). Espelha codex:* nos 4
  // canais de auth/diagnostico. Sem canais de warning/prep (Kimi nao tem aqui).
  kimi: {
  ...
  // namespace claude-cli:* (runtime NATIVO Claude Code). Espelha codex:*/kimi:* nos 4
  // canais de diagnostico/config. O Claude nasceu default e nunca teve menu; aqui o
  // usuario consulta status/versao/auth, testa a conexao e aponta um path customizado
  // do binario (caso o engine empacotado nao resolva, ex: node_modules em outro HD).
  claudeCli: {
  ...
  openDesign: {
    ...
    setup: (projectId: string, config: Record<string, unknown>) => ipcRenderer.invoke('open-design:setup', projectId, config),
    ...
    // Sprint 4 (SPEC L858-869): `openDesign.lock` REMOVIDO do preload publico.
    // Lock e interno do PipelineEngine. Renderer/DevTools nao podem bypassar.
    // Use `window.lionclaw.pipeline.approve(projectId, { action: 'lock-and-continue' })`
    // na fase 4 do pipeline development-v2.
    ...
    // Sprint 3: WebContentsView bridge
    ...
    // Sprint 1: boot installer (global)
    ...
    // Sprint 2 (SPEC L591-737): session config + bootstrap.
    getSessionConfig: (projectId: string) =>
      ipcRenderer.invoke('open-design:get-session-config', projectId),
    setSessionConfig: (
      projectId: string,
      cfg: import('../../src/types/open-design').OpenDesignSessionConfig,
    ) => ipcRenderer.invoke('open-design:set-session-config', projectId, cfg),
    ensureSession: (projectId: string) =>
      ipcRenderer.invoke('open-design:ensure-session', projectId),
    // (A2-bis) status de inicio da fase OD: { driveEngaged, startPending } pelos
    // predicados canonicos do main. O renderer decide auto-iniciar vs CTA.
    getStartStatus: (projectId: string) =>
      ipcRenderer.invoke('open-design:get-start-status', projectId),
    onBootInstallStream: (handler: (event: import('../../src/types/open-design').BootInstallStreamEvent) => void): (() => void) => {
      ...
      return (): void => { ipcRenderer.removeListener('open-design:boot-install-stream', wrapped); };
    },
    onBootstrapProgress: (handler: (event: import('../../src/types/open-design').OpenDesignBootstrapProgressEvent) => void): (() => void) => {
      const wrapped = (_event: unknown, payload: unknown): void => {
      ...
      return (): void => { ipcRenderer.removeListener('open-design:bootstrap-progress', wrapped); };
    },
  },
  pricing: {
    calculate: (input) => ipcRenderer.invoke('pricing:calculate', input),
  },
  // SPEC orchestrator-pipeline-control (S8): driver do orquestrador. Read/ops do
  // drive + listener `drive:state-changed` (broadcast do coordenador, com cleanup).
  drive: {
    getState: (projectId: string) => ipcRenderer.invoke('drive:get-state', projectId),
    start: (projectId: string, mode: 'semi' | 'full') =>
      ipcRenderer.invoke('drive:start', projectId, mode),
    ...
    resume: (projectId: string) => ipcRenderer.invoke('drive:resume', projectId),
    setMode: (projectId: string, mode: 'semi' | 'full') =>
      ipcRenderer.invoke('drive:set-mode', projectId, mode),
    onStateChanged: (
      cb: (payload: { projectId: string; drive: DriveState | null }) => void,
    ): (() => void) => {
      const handler = (_: Electron.IpcRendererEvent, payload: { projectId: string; drive: DriveState | null }) =>
        cb(payload);
      ipcRenderer.on('drive:state-changed', handler);
      return () => { ipcRenderer.removeListener('drive:state-changed', handler); };
    },
  },
  // SPEC spec-chat-repo-codegraph.md (secao 6): repo mode com CodeGraph no chat.
  // 12 invokes (incl. `repo-graph:metrics`, D-5/A4) + listener
  // `repo-graph:on-status` (broadcast do engine, com cleanup). build/update sao
  // o UNICO caminho ate o writer (5.2/13.1).
  repoGraph: {
    list: () => ipcRenderer.invoke('repo-graph:list'),
    addRepository: (rawPath: string) =>
      ipcRenderer.invoke('repo-graph:add-repository', rawPath),
    removeRepository: (repositoryId: string) =>
      ipcRenderer.invoke('repo-graph:remove-repository', repositoryId),
    getSessionState: (sessionId: string) =>
      ipcRenderer.invoke('repo-graph:get-session-state', sessionId),
    ...
    detachSession: (sessionId: string) =>
      ipcRenderer.invoke('repo-graph:detach-session', sessionId),
    ...
    status: (repositoryId: string) =>
      ipcRenderer.invoke('repo-graph:status', repositoryId),
    ...
    onStatus: (cb: (event: RepoGraphStatusEvent) => void): (() => void) => {
      ...
      return () => { ipcRenderer.removeListener('repo-graph:on-status', handler); };
    },
  },
  // SPEC spec-kanban-nativo-2026-08-31.md (secao 5): Kanban nativo por
  // repositorio. 16 invokes `kanban:*` + listener do broadcast `kanban:changed`
  // (emitido pelo engine apos QUALQUER escrita — UI, orquestrador ou scheduler;
  // cleanup obrigatorio). Assinaturas em KanbanAPI (src/types/kanban.ts).
  kanban: {
    ...
    updateCard: (board, localId, patch) =>
      ipcRenderer.invoke('kanban:update-card', board, localId, patch),
    ...
    unarchiveCard: (board, localId) =>
      ipcRenderer.invoke('kanban:unarchive-card', board, localId),
    deleteCard: (board, localId, hard) =>
      ipcRenderer.invoke('kanban:delete-card', board, localId, hard ?? false),
    attachFile: (board, localId, filePath) =>
      ipcRenderer.invoke('kanban:attach-file', board, localId, filePath),
    removeAttachment: (attachmentId) =>
      ipcRenderer.invoke('kanban:remove-attachment', attachmentId),
  ...
  // SPEC-010 Dynamic Workflow Pipe (secao 14): namespace completo upfront (S01).
  // 23 invokes `dynamic-workflow:*` (D25c: +get-run-bundle/open-run-dir) + listener do broadcast
  // `dynamic-workflow:stream` (onEvent, com cleanup - padrao drive acima).
  // Assinaturas em DynamicWorkflowAPI (src/types/dynamic-workflow.ts). Criacao
  // saiu do renderer (exclusiva do orquestrador via MCP, refatoracao 2026-08-27).
  dynamicWorkflow: {
    ...
    intervene: (runId, intervention) =>
      ipcRenderer.invoke('dynamic-workflow:intervene', runId, intervention),
    requestReplan: (runId, request) =>
      ipcRenderer.invoke('dynamic-workflow:request-replan', runId, request),
    approveGate: (runId, gateId, decision) =>
      ipcRenderer.invoke('dynamic-workflow:approve-gate', runId, gateId, decision),
    resolveWithCloser: (runId, reason) =>
      ipcRenderer.invoke('dynamic-workflow:resolve-with-closer', runId, reason),
    sendCloserMessage: (runId, message) =>
      ipcRenderer.invoke('dynamic-workflow:closer-message', runId, message),
    finalizeWorkflow: (runId) =>
      ipcRenderer.invoke('dynamic-workflow:finalize', runId),
    getRun: (runId) => ipcRenderer.invoke('dynamic-workflow:get-run', runId),
    getSnapshot: (runId) =>
      ipcRenderer.invoke('dynamic-workflow:get-snapshot', runId),
    listRuns: () => ipcRenderer.invoke('dynamic-workflow:list-runs'),
    getNodes: (runId) => ipcRenderer.invoke('dynamic-workflow:get-nodes', runId),
    // D23: `opts` opcional ({afterSeq, beforeSeq, limit, types}); sem opts o main
    // devolve os ultimos 1000 (alias compativel).
    getEvents: (runId, opts) => ipcRenderer.invoke('dynamic-workflow:get-events', runId, opts),
    // D25c: pacote do run (listagem so-leitura) e abrir a pasta do run no SO. O
    // runDir e resolvido no main; o renderer nunca monta caminho.
    ...
    getArtifacts: (runId) =>
      ipcRenderer.invoke('dynamic-workflow:get-artifacts', runId),
    getMessages: (runId) =>
      ipcRenderer.invoke('dynamic-workflow:get-messages', runId),
    onEvent: (cb: (chunk: DynamicWorkflowStreamChunk) => void): (() => void) => {
      ...
      return () => { ipcRenderer.removeListener('dynamic-workflow:stream', handler); };
    },
  },
};
