...
describe('contexto-vivo §4 — onToolUseIO (input E output separados)', () => {
  it('input stashado no in_progress + rawOutput do completed chegam SEPARADOS', () => {
    ...
    expect(onToolUseComplete).toHaveBeenCalledWith('read_file', { path: '/tmp/a.txt' });
  });
  ...
  it('rawInput que chega no proprio in_progress (nao no tool_call) tambem e separado', () => {
    ...
    translateSessionUpdate(
      { sessionUpdate: 'tool_call', toolCallId: 'tc2', title: 'bash' },
      acc,
    ...
    expect(onToolUseIO).toHaveBeenCalledWith('bash', { cmd: 'ls' }, 'a b c');
  });
  ...
  it('sem rawInput stashado: input fica undefined e o output NAO colapsa pro lado do input', () => {
    ...
    expect(onToolUseComplete).toHaveBeenCalledWith('tool', { r: 1 });
    expect(onToolUseIO).toHaveBeenCalledWith('tool', undefined, { r: 1 });
  });
  ...
  it('callback ausente = no-op (callers antigos byte-identicos)', () => {
    ...
    expect(() => {
      translateSessionUpdate(
        { sessionUpdate: 'tool_call', toolCallId: 't', title: 'x', rawInput: 1 },
        acc,
    ...
    expect(onToolUseComplete).toHaveBeenCalledWith('x', 1);
  });
});
