...
describe("codex stream-translator (SP-9.10)", () => {
  it("emits only existing StreamChunk types and drops onReasoning", async () => {
    const { createCodexStreamTranslator } =
      await import("../stream-translator");
    ...
    expect(
      emitted.some(
        (c) => c.type === ("thinking" as unknown as StreamChunk["type"]),
      ),
    ).toBe(false);
    expect(
      emitted.some(
        (c) => c.type === ("tool_start" as unknown as StreamChunk["type"]),
      ),
    ).toBe(false);
...
describe("codex stream-translator: atividade de tools MCP (BUG 1)", () => {
  ...
  it("duas chamadas MCP concorrentes concluidas em ordem INVERSA fecham cada uma a sua entrada", async () => {
    ...
    const startA = activityChunks.find(
      (c) => c.activity?.phase === "start" && c.activity?.label === "mcp:x.a",
    );
    const startB = activityChunks.find(
      (c) => c.activity?.phase === "start" && c.activity?.label === "mcp:x.b",
    );
    const endA = activityChunks.find(
      (c) => c.activity?.phase === "end" && c.activity?.label === "mcp:x.a",
    );
    const endB = activityChunks.find(
      (c) => c.activity?.phase === "end" && c.activity?.label === "mcp:x.b",
    );
    ...
    const callA = emitted.find(
      (c) => c.type === "tool_call" && c.tool === "mcp:x.a",
    );
    const callB = emitted.find(
      (c) => c.type === "tool_call" && c.tool === "mcp:x.b",
    );
    const resultA = emitted.find(
      (c) => c.type === "tool_result" && c.tool === "mcp:x.a",
    );
    const resultB = emitted.find(
      (c) => c.type === "tool_result" && c.tool === "mcp:x.b",
    );
