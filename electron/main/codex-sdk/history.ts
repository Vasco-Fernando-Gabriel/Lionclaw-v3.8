...
export function buildCodexHistoryPreamble(
  ...
  for (let i = priorTurns.length - 1; i >= 0; i -= 1) {
    ...
    if (selected.length > 0 && usedChars + nextChars > CODEX_HISTORY_MAX_CHARS) {
      break;
    }

    if (selected.length === 0 && nextChars > CODEX_HISTORY_MAX_CHARS) {
      selected.unshift(formatted.slice(0, CODEX_HISTORY_MAX_CHARS));
      break;
    }
