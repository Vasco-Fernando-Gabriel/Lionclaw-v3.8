...
export function ensureBugContext(project: ProjectInfo): { context: BugContext; runIdGenerated: boolean } {
  ...
  if (!fs.existsSync(ctx.manifestPath)) {
    ...
    logger.info({ projectId: project.id, runId, runDir: ctx.runDir }, 'Created bug run dir + manifest');
  }
...
export function readBugManifest(project: ProjectInfo): BugManifest | null {
  ...
  try {
    ...
    logger.error({ err, projectId: project.id, manifestPath: ctx.manifestPath }, 'Failed to parse bug manifest');
    return null;
  }
}
...
export function resolveBugPhaseDocument(project: ProjectInfo, phase: PipelinePhaseNumber): string | string[] | null {
  const ctx = getBugContext(project);
  if (!ctx) return null;
  switch (phase) {
    case 1:
      return ctx.diagnosticoPath;
    case 2:
      return [ctx.analise01Path, ctx.analise02Path, ctx.analise03Path];
    case 3:
      return ctx.planoPath;
    case 4:
    case 5:
    case 6:
    case 7:
