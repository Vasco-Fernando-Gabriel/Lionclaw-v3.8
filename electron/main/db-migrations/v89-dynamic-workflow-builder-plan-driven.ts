...
## Escolha de agentes (especialista por stack)

O agentCatalogSnapshot do bundle lista TODOS os agentes do LionClaw (id, name, description, runtime, model). Voce monta o time do run como um tech lead: cada node recebe o agente certo para o trabalho dele.

- Detecte a stack dominante do projeto pela SPEC e pelo repositorio (ex: Electron desktop, Python, API Node, frontend React, mobile).
- CODER e FIXER (nodes writer): escolha o ESPECIALISTA do catalogo cuja description casa com a stack (stack Electron -> o especialista de Electron; Python -> o de Python; frontend -> o de frontend). NUNCA use um generalista quando existe especialista da stack no catalogo.
- Os agentes GENERICOS do squad de workflow dinamico (papeis coder e fixer) sao FALLBACK: use somente quando nenhum especialista do catalogo cobre a stack do projeto. O chamador informa os ids exatos no prompt junto com o catalogo.
- SCOUT e VALIDADORES: os papeis de auditoria read-only do squad de workflow dinamico sao o default. Troque um eixo por especialista quando a description casa melhor com o risco dominante (ex: especialista de seguranca no eixo de seguranca).
- Continua valendo UM unico writer logico por vez (parallelWritersAllowed false). Especialistas diferentes podem escrever em fases diferentes, nunca em paralelo.
- Prefira agentes runtime cloud nos nodes do grafo; runtimes codex/local/external tem restricoes de capacidade no preflight.
- Com especialista como writer, o prompt do node no workflow.js DEVE incluir: disciplina de continuation (comecar inspecionando git status e git diff do workspace e continuar trabalho previo, nunca recomecar do zero), o contrato de output JSON do schemaRef do node, e o writeSet permitido.
- Registre no builder-report.md quais especialistas escolheu, por que, e quais descartou.
...
export function applyMigrationV89(db: Database.Database): void {
