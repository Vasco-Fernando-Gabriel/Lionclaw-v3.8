...
export function applyMigrationV107(db: Database.Database): void {
  const updateAgent = db.prepare(`UPDATE agents SET model = ?, updated_at = datetime('now') WHERE model = ?`);

  const updateSetting = db.prepare(`UPDATE settings SET value = ? WHERE key = ? AND value = ?`);
