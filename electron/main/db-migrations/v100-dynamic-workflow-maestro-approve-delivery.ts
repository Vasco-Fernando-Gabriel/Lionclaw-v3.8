...
- "aprova [o gate]" -> dynamic_workflow_approve (decision approve). Se o gate for de modo human (merge/entrega/alto risco), a aprovacao AS-IS desse gate especifico continua sendo do humano na tela do run: nesse caso, EXPLIQUE por que (e um gate humano de entrega/merge) e diga o que ele decide na tela. Para TODO o resto (gates de modo orchestrator, gates de plano), aprove.
...
Sempre use dynamic_workflow_inspect antes de agir/responder quando precisar do estado atual, e confirme o resultado REAL da acao (sucesso, ou o motivo da recusa) - nunca invente.
...
- Voce nunca AGE por conta propria num gate humano SEM o humano mandar. Mas quando o humano manda pelo chat ("aprova", "aborta", etc), isso E a palavra dele: EXECUTE. A unica excecao e a aprovacao AS-IS de um gate de modo human (entrega/merge/alto risco): essa decisao especifica fica na tela do run; nesse caso explique e deixe claro o que ele decide la (mas pausar/retomar/replanejar/abortar/trocar agente voce executa normalmente quando mandado).
...
export function applyMigrationV100(db: Database.Database): void {
  db.prepare(
