...
import { listProviderStatuses, invalidateProviderStatusCache } from '../provider-availability';
...
describe('provider:connect side-effects', () => {
  it('Z.ai: writes to vault under ORCHESTRATOR_ZAI_API_KEY and persists the ref setting', async () => {
    ...
    expect(settingsStore.get('orchestrator_zai_api_key_ref')).toBe('ORCHESTRATOR_ZAI_API_KEY');
  });
  ...
  it('MiniMax: writes to vault under ORCHESTRATOR_MINIMAX_API_KEY and persists the ref setting', async () => {
    ...
    setSetting('orchestrator_minimax_api_key_ref', 'ORCHESTRATOR_MINIMAX_API_KEY');

    expect(mockedSetSecret).toHaveBeenCalledWith('ORCHESTRATOR_MINIMAX_API_KEY', 'sk-test-minimax');
    expect(mockedSetSetting).toHaveBeenCalledWith('orchestrator_minimax_api_key_ref', 'ORCHESTRATOR_MINIMAX_API_KEY');
    expect(vaultStore.get('ORCHESTRATOR_MINIMAX_API_KEY')).toBe('sk-test-minimax');
    expect(settingsStore.get('orchestrator_minimax_api_key_ref')).toBe('ORCHESTRATOR_MINIMAX_API_KEY');
  ...
  it('MiniMax connect without apiKey: handler returns { error } and NEVER throws', async () => {
    async function simulateConnect(apiKey: string | undefined): Promise<{ ok: true } | { error: string }> {
      if (!apiKey || apiKey.trim().length === 0) {
  ...
  it('OpenAI-compatible: writes vault key + base URL + preset', async () => {
    ...
    expect(settingsStore.get('orchestrator_openai_compat_base_url')).toBe('https://api.deepseek.com');
  ...
  it('Ollama / LM Studio: only base URL setting, no vault interaction', async () => {
    ...
    expect(settingsStore.get('orchestrator_ollama_base_url')).toBe('http://localhost:11434');
    expect(settingsStore.get('orchestrator_lmstudio_base_url')).toBe('http://localhost:1234');
    expect(mockedSetSecret).not.toHaveBeenCalled();
  });
});
...
describe('provider:disconnect side-effects', () => {
  ...
  it('MiniMax: removes vault entry + clears settings ref', async () => {
    ...
    expect(mockedDeleteSecret).toHaveBeenCalledWith('ORCHESTRATOR_MINIMAX_API_KEY');
    expect(mockedSetSetting).toHaveBeenCalledWith('orchestrator_minimax_api_key_ref', '');
    expect(vaultStore.has('ORCHESTRATOR_MINIMAX_API_KEY')).toBe(false);
...
describe('cache invalidation', () => {
  it('after invalidateProviderStatusCache, the next listProviderStatuses call probes fresh', async () => {
    ...
    const firstZai = first.find((s) => s.runtime === 'claude-compat-sdk' && s.provider === 'zai');
    ...
    const secondZai = second.find((s) => s.runtime === 'claude-compat-sdk' && s.provider === 'zai');
...
describe('settings:get/update — orchestratorMinimaxApiKeyRef', () => {
  ...
  it('settings:update propagates orchestratorMinimaxApiKeyRef to the snake_case setting', () => {
    ...
    if (incoming.orchestratorMinimaxApiKeyRef !== undefined) {
      setSetting('orchestrator_minimax_api_key_ref', incoming.orchestratorMinimaxApiKeyRef);
    }

    expect(getSetting('orchestrator_minimax_api_key_ref')).toBe('new-value');
    expect(mockedSetSetting).toHaveBeenCalledWith('orchestrator_minimax_api_key_ref', 'new-value');
...
describe('SPEC-004 §5.8: COMPAT_VAULT_REFS source-level guardrail', () => {
  it('ipc-handlers.ts declares COMPAT_VAULT_REFS with zai + minimax entries', async () => {
    ...
    const src = fs.readFileSync(path.join(__dirname, '..', 'ipc-handlers.ts'), 'utf8');
    ...
    expect(src).toMatch(/payload\.provider === 'zai' \|\| payload\.provider === 'minimax'/);
