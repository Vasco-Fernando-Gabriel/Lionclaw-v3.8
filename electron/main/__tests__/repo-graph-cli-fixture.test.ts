...
const requirePhysicalRuntime =
  process.env.LIONCLAW_REQUIRE_CODEGRAPH_RUNTIME === "1";
...
function runCli(args: string[]): string {
  const {
    ...
    runAsNode,
    windowsHide,
  } = resolveCodegraphSpawn(binary, args);
...
(resolvedPhysicalBinary ? describe : describe.skip)(
  "gate de fixture da CLI codegraph (17.1.1, versao pinada)",
  () => {
    it(
      ...
      () => {
        runCli(["init", "--index"]);
        expect(
          fs.existsSync(path.join(fixtureDir, ".codegraph", "codegraph.db")),
        ).toBe(true);
      },
      CMD_TIMEOUT_MS,
    );
    ...
    it(
      ...
      () => {
        ...
        expect(() =>
          runCli(["impact", "add", "--limit", "5", "--json"]),
        ).toThrow();
      },
      CMD_TIMEOUT_MS,
    );
...
describe("disponibilidade da closure física CodeGraph", () => {
  (requirePhysicalRuntime ? it : it.skip)(
    "é obrigatória nos jobs nativos de package",
    () => {
      expect(
        resolvedPhysicalBinary,
        "closure física target-specific do CodeGraph ausente",
      ).toBeTruthy();
    },
  );
});
