...
describe('caminho tudo-cabe (AC-34)', () => {
  it('formato fixado por fixture: [role] content unido por \\n\\n, byte-identico, zero maps', async () => {
    ...
    expect(r.messageText).toBe('[user] primeira mensagem\n\n[assistant] primeira resposta\n\n[user] segunda pergunta');
...
describe('map de mensagem gigante (AC-35)', () => {
  it('mensagem de 120k chars vira UM bloco com marcador, <= T_msg, via map+reduce', async () => {
    ...
    const giant = 'g'.repeat(120000);
    const r = await buildBudgetedMessageText([msg('user', giant), msg('assistant', 'resposta curta')], 48000, {
      kind: 'claude',
      invoker,
      clampBudgetTokens: 48000,
    });
  ...
  it('warn de modelo local (11.6): input acima do limite configuravel loga warn', async () => {
    ...
    expect(h.logWarn.mock.calls.some((c) => String(c[1]).includes('compaction_local_input_warn_tokens'))).toBe(true);
  });
});
...
describe('FOLD recency-preserving (AC-36)', () => {
  ...
  it('delta pior-caso (~600k chars) conclui <= budget*1.05 com cauda verbatim', async () => {
    const fake = makeFakeInvoker((p) => (p.includes('RESUMOS PARCIAIS') ? 'REDUCE-X' : 'RES-SEG'));
    const messages = Array.from({ length: 60 }, (_, i) => msg('user', `DOC${i} ${'d'.repeat(9990)}`));
    ...
    expect(r.messageText).toContain('DOC59');
    expect(r.stats.mapCalls).toBeLessThanOrEqual(12);
    expect(r.stats.usedLegacyAssembly).toBe(false);
  });
});
...
describe('caps e timeout (fallback nivel 2)', () => {
  it('cap de chamadas: segmentos restantes viram excerpt deterministico + logger.error', async () => {
    ...
    const g1 = 'a'.repeat(30000);
    const g2 = 'b'.repeat(30000) + 'FIM-G2';
    const r = await buildBudgetedMessageText([msg('user', g1), msg('user', g2), msg('assistant', 'ok')], 48000, {
      kind: 'claude',
      invoker,
      clampBudgetTokens: 48000,
      mapCallLimit: 1,
    ...
    expect(r.messageText).toContain('FIM-G2');
    expect(r.stats.deterministicFallbacks).toBeGreaterThanOrEqual(1);
    expect(h.logError.mock.calls.some((c) => String(c[1]).includes('caps de map estourados'))).toBe(true);
  });
  ...
  it('timeout por chamada (Promise.race): bloco degrada, ciclo conclui', async () => {
    ...
    const r = await buildBudgetedMessageText([msg('user', giant), msg('assistant', 'seguimos')], 48000, {
      kind: 'claude',
      invoker,
      clampBudgetTokens: 48000,
      mapTimeoutMs: 10,
    ...
    expect(r.stats.deterministicFallbacks).toBeGreaterThanOrEqual(1);
    expect(h.logWarn.mock.calls.some((c) => String(c[1]).includes('fallback nivel 1'))).toBe(true);
  });
});
...
describe('escada de fallback (AC-38/AC-39)', () => {
  it('nivel 1: falha de UM map degrada so aquele bloco; os demais seguem', async () => {
    ...
    const r = await buildBudgetedMessageText([msg('user', gA), msg('user', gB)], 48000, {
      kind: 'claude',
      invoker,
      clampBudgetTokens: 48000,
    });
    ...
    expect(r.messageText).toContain('RESUMO-B');
...
describe('enforcement pos-hoc do output (kind subscription)', () => {
  it('output acima de 2x o alvo e reduzido ao excerpt do proprio output + warn', async () => {
    ...
    expect(h.logWarn.mock.calls.some((c) => String(c[1]).includes('enforcement pos-hoc'))).toBe(true);
  });
...
describe('clamp final de seguranca', () => {
  it('messageText acima de budget*1.05 poda os blocos mais ANTIGOS com marcador + logger.error', async () => {
    const { invoker } = makeFakeInvoker(() => 'R'.repeat(24000));
    ...
    expect(h.logError.mock.calls.some((c) => String(c[1]).includes('clamp final disparou'))).toBe(true);
...
describe('summarizePlainBlock', () => {
  ...
  it('resposta vazia lanca (o fallback e decisao do caller)', async () => {
    const { invoker } = makeFakeInvoker(() => '');
    await expect(summarizePlainBlock({ invoker, kind: 'claude', text: 'abc', targetTokens: 100 })).rejects.toThrow(
      'resposta vazia',
    );
  });
});
