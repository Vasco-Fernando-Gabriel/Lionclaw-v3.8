...
describe('chat-context invariantes GENUINOS: frozen paths (parte A, git)', () => {
  for (const frozen of FROZEN_PATHS) {
    (gitAvailable ? it : it.skip)(`byte-identico vs base: ${frozen}`, () => {
      ...
      expect(
        diff,
        `zona sagrada tocada pela campanha chat-context: ${frozen}\n${diff}`,
      ).toBe('');
    });
  }
  ...
  (gitAvailable && FROZEN_PATHS.length > 0 ? it : it.skip)(
    ...
    () => {
      const diff = execFileSync(
        'git',
        ['diff', '--stat', BASE_REF as string, '--', ...FROZEN_PATHS],
...
function removedLines(p: string): string[] {
...
describe('chat-context orchestrator.ts ADITIVO (parte B, git)', () => {
  (gitAvailable ? it : it.skip)(
    'nenhuma linha REMOVIDA toca a zona sagrada de executeClaudeSdkQuery',
    () => {
      const removed = removedLines(ORCHESTRATOR);
      const offenders = removed.filter((line) =>
        FORBIDDEN_IN_REMOVED.some((tok) => line.includes(tok)),
      );
      expect(
        offenders,
...
describe('chat-context ancoras sagradas presentes (parte C, estatica)', () => {
  it('orchestrator.ts ainda contem os marcadores verbatim de query()/replay/thread', () => {
    ...
    for (const anchor of SACRED_ANCHORS) {
      expect(src.includes(anchor), `ancora sagrada sumiu do orchestrator: ${anchor}`).toBe(
        true,
      );
    }
  });
