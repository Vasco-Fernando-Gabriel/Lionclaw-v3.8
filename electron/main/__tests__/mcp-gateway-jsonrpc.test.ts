...
describe('mcp_invoke — despacho pro wrapper central', () => {
  ...
  it('args ausente vira {}; surface desconhecido normaliza p/ claude-sdk; ids sinteticos defensivos', async () => {
  ...
  it('turno desktop ativo no main SOBREPOE os ids sinteticos do helper (par atomico)', async () => {
    const { setActiveChatTurn, __resetChatCapabilityContextForTests } = await import(
      '../chat-capability-context'
    );
    try {
      ...
      expect(mockInvoke).toHaveBeenCalledWith(
        expect.objectContaining({ sessionId: 'sess-real', turnId: 'turn-real' }),
      );
    } finally {
  ...
  it('config vazio do surface -> allowedServerIds [] (o wrapper bloqueia por escopo)', async () => {
    ...
    await rpc('mcp_invoke', { server: 'google-gmail', tool: 'send_email' });
    expect(mockInvoke).toHaveBeenCalledWith(
      expect.objectContaining({ allowedServerIds: [] }),
    );
  });
  ...
  it('excecao inesperada do wrapper -> RPC error -32000 com a mensagem real', async () => {
    ...
    const res = await rpc('mcp_invoke', { server: 'shopify', tool: 'x' });
