...
describe('buildKnowledgeFtsQuery', () => {
  it('builds an FTS5-safe query for hyphenated knowledge searches', () => {
    const ftsQuery = buildKnowledgeFtsQuery(
      'LangGraph graph-based agents workflow orchestration',
    );

    expect(ftsQuery).toBe(
      '"LangGraph" OR "graph" OR "based" OR "agents" OR "workflow" OR "orchestration"',
    );
