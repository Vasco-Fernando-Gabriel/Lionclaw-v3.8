...
describe('db-migration-v48: analise estrutural do SQL', () => {
  ...
  it('CHECK constraint aceita exatamente 10 status', () => {
    const match = MIGRATION_V48_EXPAND_PROJECT_STATUS_CHECK.match(/CHECK \(status IN \(([\s\S]*?)\)\)/);
  ...
  it('preserva TODAS as 27 colunas atuais (V37 base + V41 + V42 + V45) no SELECT', () => {
    const requiredCols = [
      ...
      'discovery_notes_path',
      'prd_path',
      'pipeline_sprint_index',
      'pipeline_discovery_block',
      'pipeline_type',
  ...
  it('default do status continua sendo idle (preservacao de comportamento)', () => {
    expect(MIGRATION_V48_EXPAND_PROJECT_STATUS_CHECK).toMatch(/status TEXT NOT NULL DEFAULT 'idle'/);
  });

  it('default de pipeline_type continua sendo development (preservacao)', () => {
    expect(MIGRATION_V48_EXPAND_PROJECT_STATUS_CHECK).toContain("pipeline_type TEXT NOT NULL DEFAULT 'development'");
  });
});
...
describe('db-migration-v48: execucao em banco in-memory', () => {
  ...
  it('preserva o numero total de projetos pre-existentes apos migration', () => {
    const stmt = db.prepare(`INSERT INTO harness_projects (name, project_path, spec_path, status) VALUES (?, ?, ?, ?)`);
  ...
  it('aceita inserir projeto com status="aborted" pos-migration', () => {
    ...
    const row = db.prepare(`SELECT status FROM harness_projects WHERE name = 'post'`).get() as { status: string };
