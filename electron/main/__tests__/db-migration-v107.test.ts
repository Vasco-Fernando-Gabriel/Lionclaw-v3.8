...
const SHORT = __V107_INTERNAL.SHORT_ID; // 'kimi-for-coding'
const NS = __V107_INTERNAL.NAMESPACED_ID; // 'kimi-code/kimi-for-coding'
...
describe('V107 R10 - source edits carry the namespaced id (fresh installs)', () => {
  it('kimi-models.ts catalog uses the namespaced subscription slug', () => {
    const src = readFileSync(
      join(__dirname, '..', '..', '..', 'src', 'constants', 'kimi-models.ts'),
      'utf8',
    );
...
describe('V107 - no em-dash', () => {
  it('migration source has no U+2014', () => {
    const src = readFileSync(
      join(__dirname, '..', 'db-migrations', 'v107-kimi-model-id-fix.ts'),
      'utf8',
    );
