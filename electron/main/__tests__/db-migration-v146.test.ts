...
describe('migration v146 Cursor runtime', () => {
  it('migra SQLite real, preserva agentes/FKs/eixos e aceita cursor', () => {
    ...
    db.exec(`
      CREATE TABLE agents (
        ...
        runtime TEXT DEFAULT 'cloud' CHECK (runtime IN ('cloud','local','external','codex','zai','minimax-tp','kimi','grok')),
      ...
      INSERT INTO agents (
        ...
        'custom', 'Custom', 'desc', 'prompt', 'grok-4.6', '["Read"]', '["kb"]',
        1, 9, 'max', 'enabled', 1234, 77, '["skill"]', 0, 'grok',
  ...
  it('propagates an injected failure from inside the transaction', () => {
    ...
    expect(() => applyMigrationV146(db)).toThrow('injected migration failure');
  });
  ...
  it('preserva violacoes legadas sem atribui-las a migration', () => {
    ...
    expect(() => applyMigrationV146(db)).not.toThrow();
  });
  ...
  it('rejects new foreign-key violations before the transaction commits', () => {
    ...
    expect(() => applyMigrationV146(db)).toThrow('foreign key');
  });
});
