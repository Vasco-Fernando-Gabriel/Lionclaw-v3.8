...
} from '../seed-agents/architecture-spec-enricher';
import {
  ARCHITECTURE_REVIEW_AGENT_IDS,
...
describe('architecture-spec-enricher seed agent', () => {
  it('is registered as an architecture-review seed agent', () => {
    ...
    expect(ARCHITECTURE_REVIEW_SEED_AGENTS.map((agent) => agent.id))
      .toContain(ARCHITECTURE_SPEC_ENRICHER_ID);
    expect(ALL_SEED_AGENTS.map((agent) => agent.id))
      .toContain(ARCHITECTURE_SPEC_ENRICHER_ID);
  });
