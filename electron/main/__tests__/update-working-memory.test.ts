...
async function updateWorkingMemoryTest(
  ...
  if (nonEmptyCount > 50) {
    ...
    if (pruneIdx === -1 || blocks[pruneIdx].lines.filter(l => l.trim().length > 0).length === 0) {
      warnFn('updateWorkingMemory: MEMORY.md over 50 non-empty lines but "Estado de projetos" is empty — cannot prune other sections', { nonEmptyCount, limit: 50 });
    } else {
      const pruneBlock = blocks[pruneIdx];
      while (nonEmptyCount > 50) {
        const firstNonEmpty = pruneBlock.lines.findIndex(l => l.trim().length > 0);
        if (firstNonEmpty === -1) {
          warnFn('updateWorkingMemory: "Estado de projetos" exhausted but still over 50 non-empty lines — stopping prune', { nonEmptyCount, limit: 50 });
          break;
        }
...
describe('updateWorkingMemory (Sprint 3)', () => {
  ...
  it('4. 55 linhas nao-vazias -> poda do COMECO de "Estado de projetos" ate caber em 50', async () => {
    const decisoesLines = Array.from({ length: 10 }, (_, i) => `- [2026-01-${String(i + 1).padStart(2, '0')}] Decisao ${i + 1}`);
    const workaroundsLines = Array.from({ length: 5 }, (_, i) => `- [2026-01-${String(i + 1).padStart(2, '0')}] Workaround ${i + 1}`);
    const estadoLines = Array.from({ length: 35 }, (_, i) => `- [2026-01-${String(i + 1).padStart(2, '0')}] Projeto ${i + 1}`);
  ...
  it('6. MEMORY.md com 50 linhas nao-vazias + 10 blanks no meio = nao poda', async () => {
    ...
    expect(finalCount).toBeLessThanOrEqual(50);
    const prunedWarns = warn.mock.calls.filter(call =>
      typeof call[0] === 'string' && call[0].includes('pruned oldest'),
    );
