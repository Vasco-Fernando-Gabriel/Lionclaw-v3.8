...
import { applyMigrationV128, __V128_INTERNAL } from '../db-migrations/v128-tool-script-settings';
import {
...
describe('applyMigrationV128 - seed dos settings tool_script_*', () => {
  it('cria os 6 settings com os defaults da spec', () => {
    ...
    expect(settingValue(sqlite, 'tool_script_timeout_ms')).toBe(String(TOOL_SCRIPT_DEFAULT_TIMEOUT_MS));
    expect(settingValue(sqlite, 'tool_script_max_stdout_bytes')).toBe(String(TOOL_SCRIPT_DEFAULT_MAX_STDOUT_BYTES));
    expect(settingValue(sqlite, 'tool_script_max_stderr_bytes')).toBe(String(TOOL_SCRIPT_DEFAULT_MAX_STDERR_BYTES));
    expect(settingValue(sqlite, 'tool_script_max_tool_calls')).toBe(String(TOOL_SCRIPT_DEFAULT_MAX_TOOL_CALLS));
  });
  ...
  it('tool_script_tools e JSON valido com as 7 tools default', () => {
    ...
    expect(parsed).toEqual(['read_file', 'write_file', 'edit', 'grep', 'search_files', 'run_command', 'mcp_invoke']);
  });
  ...
  it('nao toca chaves alheias da tabela settings', () => {
    const { sqlite, db } = makeDb();
    sqlite.prepare('INSERT INTO settings (key, value) VALUES (?, ?)').run('mcp_prompt_mode', 'index');
    applyMigrationV128(db);
...
describe('applyMigrationV128 - idempotencia', () => {
  ...
  it('NUNCA sobrescreve customizacao do usuario', () => {
    const { sqlite, db } = makeDb();
    sqlite.prepare('INSERT INTO settings (key, value) VALUES (?, ?)').run('tool_script_enabled', 'false');
    sqlite.prepare('INSERT INTO settings (key, value) VALUES (?, ?)').run('tool_script_max_tool_calls', '10');
...
describe('V128 - integracao estatica com db.ts', () => {
  ...
  it('db.ts importa a applyMigrationV128 do arquivo v128-tool-script-settings', () => {
    expect(dbSource).toMatch(/import \{ applyMigrationV128 \} from '\.\/db-migrations\/v128-tool-script-settings'/);
  });
  ...
  it('db.ts aplica a V128 no runner (if < 128 + INSERT schema_version 128)', () => {
    ...
    expect(dbSource).toMatch(/INSERT INTO schema_version \(version\) VALUES \(\?\)'\)\.run\(128\)/);
  });
