...
describe('AC-B4b [INV] — turno vazio do orquestrador (path D6)', () => {
  it('AC-B4b: caminho de sucesso preserva limpeza, persistencia e conclusao do turno', () => {
    ...
    expect(happyPath).toContain("insertMessage(sessionId, 'assistant', assistantContent, options.agentId, messageMetadata);");
    expect(happyPath).toContain('recordCompletedMainChatTurn(sessionId, getWindow);');
    expect(happyPath.indexOf('insertMessage(')).toBeLessThan(
      happyPath.indexOf('recordCompletedMainChatTurn('),
    );
  });
  ...
  it('AC-B4b: o branch NAO aborta o ciclo do turno (sem return/throw dentro do else)', () => {
    ...
    const branch = orchestratorSrc.slice(start, orchestratorSrc.indexOf('}', orchestratorSrc.indexOf('sendSessionStream({ type: \'error\', code: emptyTurnError.code', start)));
