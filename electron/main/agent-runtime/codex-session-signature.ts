...
export function registerCodexSessionSignature(
  session: object,
  signature: CodexSessionSignature,
): void {
  signatures.set(session, signature);
}

export function getCodexSessionSignature(
  session: object,
): CodexSessionSignature | undefined {
  return signatures.get(session);
}

export function registerCodexUsageSemantics(
  session: object,
  semantics: CodexUsageSemantics,
): void {
...
export function settleCodexBilledUsage(
  session: object,
  current: CodexBilledUsageBaseline,
): CodexBilledUsageBaseline {
