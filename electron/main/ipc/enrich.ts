...
export function mergeTaskExecutionMetrics(metrics: EnrichMetrics, rollup: TaskExecutionRollup): EnrichMetrics {
  const costStatus =
    rollup.costStatus && COST_STATUS_RANK[rollup.costStatus] > COST_STATUS_RANK[metrics.costStatus ?? 'known']
      ? rollup.costStatus
      : metrics.costStatus;
  const tokenStatus =
    metrics.tokenStatus === 'not_reported' || rollup.tokenStatus === 'not_reported'
      ? 'not_reported'
      : metrics.tokenStatus;
  ...
  const entireCostIsSubscriptionEquivalent = totalCost > 0 && Math.abs(totalCost - subscriptionEquivalentCost) <= 1e-9;
  return {
    ...metrics,
    ...
    costEstimationKind: entireCostIsSubscriptionEquivalent ? 'subscription-equivalent-payg' : undefined,
