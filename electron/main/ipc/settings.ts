...
import { compactActiveChatSession } from './_shared/chat-compaction';
import type { CompactActiveChatSessionResult } from './_shared/chat-compaction';
...
export function registerSettingsHandlers(ctx: IpcContext): void {
  const { getMainWindow, getHarnessEngine } = ctx;

  ipcMain.handle('settings:get', async (): Promise<AppSettings> => {
    const orchestratorRuntime =
      (getSetting(
        'orchestrator_runtime',
      ) as AppSettings['orchestratorRuntime']) || PRODUCT_DEFAULT_ORCHESTRATOR.runtime;
    const orchestratorProvider =
      (getSetting(
        'orchestrator_provider',
      ) as AppSettings['orchestratorProvider']) || PRODUCT_DEFAULT_ORCHESTRATOR.provider;
    const orchestratorModel =
      getSetting('orchestrator_model') || PRODUCT_DEFAULT_ORCHESTRATOR.model;
    const orchestratorEffort =
      (getSetting('orchestrator_effort') as AppSettings['orchestratorEffort']) ||
      'high';
    const orchestratorCodexEffort =
      (getSetting(
        'orchestrator_codex_effort',
      ) as AppSettings['orchestratorCodexEffort']) || 'high';
    const orchestratorKimiEffort =
      (getSetting('orchestrator_kimi_effort') as AppSettings['orchestratorKimiEffort']) ||
      'max';
    const orchestratorGrokEffort =
      (getSetting('orchestrator_grok_effort') as AppSettings['orchestratorGrokEffort']) ||
      'high';
    const chatWidthModeRaw = getSetting('chat_width_mode');
    const chatWidthMode: AppSettings['chatWidthMode'] =
      chatWidthModeRaw === 'amplo' || chatWidthModeRaw === 'full-width'
        ? chatWidthModeRaw
        : 'compacto';
    const grokBinaryPath = getSetting('grok_binary_path') || undefined;
    const grokMaxConcurrencyRaw = Number.parseInt(
      getSetting('grok_max_concurrency') || '3',
      10,
    );
    const grokMaxConcurrency = Number.isFinite(grokMaxConcurrencyRaw)
      ? Math.min(8, Math.max(1, grokMaxConcurrencyRaw))
      : 3;
    const orchestratorOllamaBaseUrl =
      getSetting('orchestrator_ollama_base_url') || undefined;
    const orchestratorLmStudioBaseUrl =
      getSetting('orchestrator_lmstudio_base_url') || undefined;
    const orchestratorOpenAiCompatPresetRaw = getSetting(
      'orchestrator_openai_compat_preset',
    );
    const orchestratorOpenAiCompatPreset =
      (orchestratorOpenAiCompatPresetRaw as AppSettings['orchestratorOpenAiCompatPreset']) ||
      undefined;
    const orchestratorOpenAiCompatBaseUrl =
      getSetting('orchestrator_openai_compat_base_url') || undefined;
    const orchestratorOpenAiCompatApiKeyRef =
      getSetting('orchestrator_openai_compat_api_key_ref') || undefined;
    const orchestratorZaiApiKeyRef =
      getSetting('orchestrator_zai_api_key_ref') || undefined;
    const orchestratorMinimaxApiKeyRef =
      getSetting('orchestrator_minimax_api_key_ref') || undefined;
    const orchestratorVertexApiKeyRef =
      getSetting('orchestrator_vertex_api_key_ref') || undefined;
    const orchestratorVertexLocation =
      getSetting('orchestrator_vertex_location') || undefined;
    const orchestratorVertexProjectId =
      getSetting('orchestrator_vertex_project_id') || undefined;
    const orchestratorVertexAuthMode =
      (getSetting('orchestrator_vertex_auth_mode') as AppSettings['orchestratorVertexAuthMode']) ||
      undefined;
    const orchestratorCompactionRuntime =
      (getSetting('orchestrator_compaction_runtime') as AppSettings['orchestratorCompactionRuntime']) ||
      undefined;
    const orchestratorCompactionProvider =
      (getSetting('orchestrator_compaction_provider') as AppSettings['orchestratorCompactionProvider']) ||
      undefined;
    const orchestratorCompactionModel =
      getSetting('orchestrator_compaction_model') || undefined;
    const orchestratorContextWindowTokensRaw = parseInt(
      getSetting('orchestrator_context_window_tokens') || '',
      10,
    );
    const orchestratorCompactionThresholdPercentRaw = parseInt(
      getSetting('orchestrator_compaction_threshold_percent') ||
        String(DEFAULT_CHAT_COMPACTION_TRIGGER_PERCENT),
      10,
    );
    const orchestratorContextWindowTokens =
      Number.isFinite(orchestratorContextWindowTokensRaw) &&
      orchestratorContextWindowTokensRaw > 0
        ? orchestratorContextWindowTokensRaw
        : undefined;
    const orchestratorCompactionThresholdPercent =
      Number.isFinite(orchestratorCompactionThresholdPercentRaw)
        ? Math.min(95, Math.max(50, orchestratorCompactionThresholdPercentRaw))
        : DEFAULT_CHAT_COMPACTION_TRIGGER_PERCENT;
    const chatCompactionTargetTokensRaw = parseInt(
      getSetting(CHAT_COMPACTION_TARGET_TOKENS_SETTING_KEY) || '',
      10,
    );
    const chatCompactionTargetTokens =
      Number.isFinite(chatCompactionTargetTokensRaw) && chatCompactionTargetTokensRaw > 0
        ...
        : DEFAULT_CHAT_COMPACTION_TARGET_TOKENS;
    const chatAutoCompactionEnabled =
      getSetting(CHAT_AUTO_COMPACTION_ENABLED_SETTING_KEY) !== 'false';
    const voiceTranscriptionModelRaw =
      getSetting('voice_transcription_model') || DEFAULT_VOICE_TRANSCRIPTION_MODEL;
    ...
    const visionProvider: VisionProvider =
      visionProviderRaw === 'openai' || visionProviderRaw === 'anthropic'
        ? visionProviderRaw
        : VISION_DEFAULT.provider;
    ...
    const { resolveToolScriptRegistration } = await import(
      '../tool-script/tool-script-availability'
    );
    const { readToolScriptSettings } = await import(
      '../tool-script/tool-script-settings'
    );
    ...
    return {
      ...
      sessionTimeoutMinutes: parseInt(
        getSetting('session_timeout') || '60',
        10,
      ),
      compactionSchedule: getSetting('compaction_schedule') || '0 23 * * *',
      maxWorkingMemoryTokens: parseInt(
        getSetting('max_memory_tokens') || '2000',
        10,
      ),
      rawMessageRetentionDays: parseInt(
        getSetting('message_retention_days') || '7',
        10,
      ),
      maxSessionTokens: parseInt(
        getSetting('max_session_tokens') || '200000',
        10,
      ),
      ...
      cartesiaSpeed: Number.parseFloat(getSetting('cartesia_speed') || String(DEFAULT_CARTESIA_SPEED)) || DEFAULT_CARTESIA_SPEED,
      ...
      subagentsPromptMode:
        getSetting('subagents_prompt_mode') === 'full' ? 'full' : 'index',
      mcpPromptMode: getSetting('mcp_prompt_mode') === 'full' ? 'full' : 'index',
      chatCapabilityGateMode:
        getSetting('chat_capability_gate_mode') === 'enforce' ? 'enforce' : 'shadow',
      ...
      ...(toolScriptDecision.available
        ? {}
        : { toolScriptAvailabilityReason: toolScriptDecision.reason }),
    ...
    async (_event, settings: Partial<AppSettings>) => {
      ...
      if (touchesTriple) {
        ...
        if (missing.length > 0) {
          ...
          logger.warn(
            { missing },
            'settings:update recusado: triple orchestrator_* incompleto',
          );
          return {
      ...
      let compactionSelectionToPersist:
        | { runtime: string; provider: string; model: string }
        | null
        | undefined;
      if (touchesCompactionTriple) {
        const mergedRuntime = settings.orchestratorCompactionRuntime
          ?? getSetting('orchestrator_compaction_runtime')
          ?? '';
        const mergedProvider = settings.orchestratorCompactionProvider
          ?? getSetting('orchestrator_compaction_provider')
          ?? '';
        const mergedModel = settings.orchestratorCompactionModel
          ?? getSetting('orchestrator_compaction_model')
          ?? '';
        const populated = [mergedRuntime, mergedProvider, mergedModel]
      ...
      const nextRuntime = settings.orchestratorRuntime ?? previousRuntime;
      const nextProvider = settings.orchestratorProvider ?? previousProvider;
      const updatesOrchestratorSelection =
        settings.orchestratorRuntime !== undefined ||
        settings.orchestratorProvider !== undefined;
      const runtimeBoundaryChanged =
        updatesOrchestratorSelection &&
        (nextRuntime !== previousRuntime || nextProvider !== previousProvider);
      let switchCompaction: CompactActiveChatSessionResult | undefined;

      if (runtimeBoundaryChanged) {
        switchCompaction = await compactActiveChatSession(
          getMainWindow,
          'orchestrator-switch',
        );
        if (switchCompaction.reason === 'compaction_failed') {
          logger.warn(
            { error: switchCompaction.error, code: 'COMPACT-SKIPPED' },
            'settings:update: compactacao da troca de orquestrador falhou; troca segue sem compactar (SB-4 P5)',
          );
        }
      }
      ...
      if (settings.sessionTimeoutMinutes)
        setSetting('session_timeout', String(settings.sessionTimeoutMinutes));
      if (settings.compactionSchedule)
        setSetting('compaction_schedule', settings.compactionSchedule);
      if (settings.maxWorkingMemoryTokens)
        setSetting(
          'max_memory_tokens',
          String(settings.maxWorkingMemoryTokens),
        );
      if (settings.rawMessageRetentionDays)
        setSetting(
          'message_retention_days',
          String(settings.rawMessageRetentionDays),
        );
      if (
        (settings as Record<string, unknown>).maxSessionTokens !== undefined
      ) {
        setSetting(
          'max_session_tokens',
          String((settings as Record<string, unknown>).maxSessionTokens),
        );
      }
      if (settings.voiceResponseEnabled !== undefined) {
        setSetting(
          'voice_response_enabled',
          settings.voiceResponseEnabled ? 'true' : 'false',
        );
      }
      ...
      if (settings.voiceLiveProvider !== undefined) {
        setSetting(
          'voice_live_provider',
          settings.voiceLiveProvider === 'cartesia' ? 'cartesia' : 'elevenlabs',
        );
      }
      ...
      if (settings.cartesiaSpeed !== undefined) {
        ...
        const safeSpeed = Number.isFinite(speed)
          ? Math.min(1.5, Math.max(0.6, speed))
          : DEFAULT_CARTESIA_SPEED;
      ...
      if (settings.subagentsPromptMode !== undefined) {
        setSetting(
          'subagents_prompt_mode',
          settings.subagentsPromptMode === 'full' ? 'full' : 'index',
        );
      }
      if (settings.mcpPromptMode !== undefined) {
        setSetting(
          'mcp_prompt_mode',
          settings.mcpPromptMode === 'full' ? 'full' : 'index',
        );
      }
      if (settings.chatCapabilityGateMode !== undefined) {
        setSetting(
          'chat_capability_gate_mode',
          settings.chatCapabilityGateMode === 'enforce' ? 'enforce' : 'shadow',
        );
      }
      ...
      if (settings.orchestratorCodexEffort !== undefined) {
        ...
        if ((CODEX_EFFORT_ORDER as readonly string[]).includes(v)) {
          ...
          logger.warn(
            { value: v },
            'orchestrator_codex_effort invalido; update ignorado',
          );
        }
      }
      ...
      if (settings.chatAutoCompactionEnabled !== undefined) {
        setSetting(
          CHAT_AUTO_COMPACTION_ENABLED_SETTING_KEY,
          settings.chatAutoCompactionEnabled ? 'true' : 'false',
        );
      }
      if (settings.orchestratorSetupCompleted !== undefined) {
        ...
        if (!wasCompleted && willBeCompleted) {
          const alreadySynced =
            getSetting('seed_agents_initial_runtime_sync_completed') === 'true';
          if (!alreadySynced) {
            try {
              const seedIds = listSeedAgentIds();
              const result = await syncAgentsToOrchestrator(
                { agentIds: seedIds, mode: 'initial-onboarding' },
                { getHarnessEngine },
              );
      ...
      if (settings.dreamingTurnBasedEnabled !== undefined) {
        setSetting(
          'dreaming_turn_based_enabled',
          settings.dreamingTurnBasedEnabled ? 'true' : 'false',
        );
      }
      ...
      if (settings.toolScriptEnabled !== undefined) {
        ...
        if (wasEnabled !== willBeEnabled) {
          try {
            const { applyToolScriptEnabledChange } = await import(
              '../tool-script/tool-script-availability'
            );
      ...
      return { success: true, compaction: switchCompaction };
    },
  );
