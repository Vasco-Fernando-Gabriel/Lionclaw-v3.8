...
export function registerOpenDesignHandlers(ctx: IpcContext): void {
  ...
  ipcMain.handle('open-design:setup', async (_event, projectId: string, payload: Record<string, unknown>) => {
    try {
      ...
      setOpenDesignConfig(projectId, sanitized as Parameters<typeof setOpenDesignConfig>[1]);
      return { ok: true };
    } catch (err) {
  ...
  ipcMain.handle('open-design:boot-install-status', async () => {
    const { getBootInstallStatus } = await import('../open-design/boot-installer');
  ...
  ipcMain.handle('open-design:get-session-config', async (_event, projectId: string) => {
    try {
      const { getSessionConfig } = await import('../open-design/session-config');
      return getSessionConfig(projectId);
    } catch (err) {
      logger.error({ err, projectId }, 'open-design:get-session-config failed');
      return null;
    }
  });

  ipcMain.handle('open-design:set-session-config', async (_event, projectId: string, cfg: unknown) => {
    try {
      const { setSessionConfig } = await import('../open-design/session-config');
      setSessionConfig(projectId, cfg as Parameters<typeof setSessionConfig>[1]);
      return { ok: true } as const;
    } catch (err) {
      logger.warn({ err, projectId }, 'open-design:set-session-config rejected');
      return { error: err instanceof Error ? err.message : String(err) };
    }
  });

  ipcMain.handle('open-design:ensure-session', async (_event, projectId: string) => {
    try {
  ...
  ipcMain.handle('open-design:get-start-status', async (_event, projectId: string) => {
    try {
  ...
  ipcMain.handle('open-design:build-initial-prompt', async (_event, projectId: string) => {
    try {
      const { buildInitialPrompt } = await import('../open-design/prompt-builder');
      return await buildInitialPrompt(projectId);
    } catch (err) {
      logger.error({ err, projectId }, 'open-design:build-initial-prompt failed');
      return { error: String(err) };
    }
  });

  ipcMain.handle('open-design:inject-initial-prompt', async (_event, projectId: string) => {
    try {
      const { buildInitialPrompt } = await import('../open-design/prompt-builder');
      ...
      logger.error({ err, projectId }, 'open-design:inject-initial-prompt failed');
      return { error: String(err) };
    }
  });
  ...
  ipcMain.handle('open-design:lock', () => {
    return {
      error: 'Use pipeline:approve na fase 5; open-design:lock e interno do PipelineEngine',
    };
  });

  ipcMain.handle('open-design:get-locked-snapshot', async (_event, projectId: string) => {
    try {
      ...
      const snapshotDir = path.join(cfg.runDir, 'open-design', 'snapshots', 'latest');
      const manifestPath = cfg.manifestPath ?? path.join(snapshotDir, 'manifest.json');
      const contractPath = cfg.contractPath ?? path.join(snapshotDir, 'design-contract.json');
      const artifactHtmlPath = cfg.artifactHtmlPath ?? path.join(snapshotDir, 'artifact', 'index.html');
      ...
      logger.error({ err, projectId }, 'open-design:get-locked-snapshot failed');
      return { error: String(err) };
    }
  });

  ipcMain.handle('open-design:destructive-unlock', async (_event, projectId: string, confirmation: string) => {
    try {
      const { destructiveUnlock } = await import('../open-design/escape-hatch');
      return await destructiveUnlock(projectId, confirmation);
    } catch (err) {
      logger.error({ err, projectId }, 'open-design:destructive-unlock failed');
      return { error: String(err) };
    }
  });

  ipcMain.handle('open-design:open-artifact', async (_event, projectId: string) => {
    try {
      const { getOpenDesignConfig } = await import('../open-design/config');
      const cfg = getOpenDesignConfig(projectId);
      if (!cfg?.runDir) return { error: 'not-locked' };
  ...
  ipcMain.handle('open-design:read-locked-html', async (_event, projectId: string) => {
    try {
      const { getOpenDesignConfig } = await import('../open-design/config');
      const cfg = getOpenDesignConfig(projectId);
      if (!cfg?.runDir) return { error: 'not-locked' };
  ...
  ipcMain.handle(
    ...
    (_event, url: string, bounds: { x: number; y: number; width: number; height: number }) => {
      try {
