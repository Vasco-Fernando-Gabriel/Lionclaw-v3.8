...
const getSession = vi.fn(
  (_id: string): { id: string; title: string | null; type: string } | null => null,
);
...
vi.mock("../../title-generator", () => ({
  ensureInitialSessionTitle: (sessionId: string, message: string) =>
    ensureInitialSessionTitle(sessionId, message),
...
const createChatKimiSession = vi.fn(async () => ({
  send: (prompt: string, cb: CliStreamCallbacks, signal: AbortSignal) =>
    sessionSendImpl(prompt, cb, signal),
  close: sessionClose,
}));
...
describe("kimi-sdk chat round-trip (mocked)", () => {
  ...
  it("stop/abort mid-run leaves no pending turn", async () => {
    const { executeKimiSdkQuery, stopKimiSdkQuery, isKimiSdkQueryActive } =
      await import("../index");
