...
export function buildKimiUsageSnapshot(
  ...
  const costUsd = tokenReported && hasKnownPricing(pricingModel)
    ? calculateCost(
        pricingModel,
        usage.inputTokens,
        usage.outputTokens,
...
function artifactDedupKey(artifact: ArtifactData): string {
  ...
  if (typeof imageBase64 === "string")
    return `${artifact.type}:inline:${imageBase64.slice(0, 80)}`;
  return `${artifact.type}:${artifact.id}`;
}

export function createKimiStreamTranslator(
  opts: KimiStreamTranslatorOptions,
): KimiStreamTranslator {
  ...
  const callbacks: CliStreamCallbacks = {
    ...
    onToolUse: (name: string) => {
      ...
      try {
        const artifact = captureToolUse(pendingToolUseId, name, {});
        emitArtifactIfAny(artifact);
      } catch (err) {
        logger.debug({ err, tool: name }, "captureToolUse threw");
      }
      emit({ type: "tool_call", tool: name, toolCallId: pendingToolUseId, input: {} });
      try {
        recordAuditEntry({
          sessionId,
          subagent,
      ...
      if (!name.startsWith("mcp:")) {
        const activityId = pendingToolUseId ?? crypto.randomUUID();
    ...
    onToolUseComplete: (name: string, result: unknown) => {
      ...
      if (!name.startsWith("mcp:")) {
        const activityId = activityIdStack.pop() ?? crypto.randomUUID();
        const record = asRecord(result);
        const command =
