...
- Local packaging orchestration for packaged LionDesign artifacts.
...
- Release artifacts keep canonical app identity (`LionDesign.app` on mac, `LionDesign.exe` inside the Windows installer); local tools-pack installs may use namespace-scoped install paths only as a developer multi-instance validation convention.
