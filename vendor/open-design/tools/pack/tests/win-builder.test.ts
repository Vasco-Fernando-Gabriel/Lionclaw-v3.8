...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "LionDesign-second-setup.exe.blockmap"),
    builtManifestPath: join(namespaceRoot, "built-app.json"),
    exePath: join(namespaceRoot, "builder", "LionDesign-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "LionDesign"),
    installedExePath: join(namespaceRoot, "runtime", "install", "LionDesign", "LionDesign.exe"),
    ...
    setupPath: join(namespaceRoot, "builder", "LionDesign-second-setup.exe"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "LionDesign", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "LionDesign.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "LionDesign.exe"), "exe\n", "utf8");
      await writeFile(
      ...
      await expect(readFile(join(paths.unpackedRoot, "LionDesign.exe"), "utf8")).resolves.toBe("exe\n");
