...
Local packaging control plane for LionDesign.
...
Release artifacts keep the canonical `LionDesign.app` bundle shape; local `tools-pack install` copies it as
`LionDesign.<namespace>.app` so developer namespaces can coexist without affecting runtime data/log/cache paths.
