...
description: >
  ...
  collage) — the same aesthetic LionDesign uses for its own marketing
...
This is the canonical LionDesign marketing-page recipe — the example
...
worked example (LionDesign itself).
...
├── inputs.example.json      # LionDesign as the worked example
...
└── assets/
    ├── *.png                # 16 collage plates (LionDesign instance)
