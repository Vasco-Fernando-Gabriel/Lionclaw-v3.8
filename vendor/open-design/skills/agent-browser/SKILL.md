...
description: |
  ...
  apps, dogfooding LionDesign previews, QA, bug hunts, or reviewing app
  quality. Prefer local LionDesign preview URLs unless the user explicitly
...
Use `agent-browser` for local LionDesign preview validation: inspect rendered
...
With the LionDesign preview at `http://127.0.0.1:17573/`, run:
...
Expected success: title `LionDesign`, current URL under `127.0.0.1:17573`,
visible LionDesign UI text in the snapshot, and a screenshot at
