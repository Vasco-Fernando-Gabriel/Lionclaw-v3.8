...
checked into the LionDesign repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any LionDesign agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for LionDesign and
...
2. Replace the contents of this directory with the upstream snapshot,
   ...
   inside `SKILL.md`'s `> **LionDesign integration.**` blockquote.
...
flow (including how LionDesign behaves when Codex is not installed).
