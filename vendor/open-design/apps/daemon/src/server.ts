...
export function createAgentRuntimeToolPrompt(
  ...
  return [
    ...
    '- `OD_BIN` is the absolute path to the LionDesign CLI script. On POSIX shells run wrappers with `"$OD_NODE_BIN" "$OD_BIN" tools ...`; do not call bare `od`, which may resolve to the system octal-dump command on Unix-like systems.',
...
function renderOAuthResultPage(opts) {
  ...
  const body = ok
    ? `Your MCP server <code>${escapeHtml(opts.serverId ?? '')}</code> is now connected. You can close this tab and return to LionDesign.`
...
<head>
<meta charset="utf-8" />
<title>${escapeHtml(title)} — LionDesign</title>
...
export async function startServer({
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    // LionDesign relays them to the agent so the model can call those tools.
  ...
  orbitService.setRunHandler(async ({
    ...
    design.runs.start(run, () => startChatRun({
      ...
      systemPrompt: [
        ...
        systemPrompt,
        'You are Orbit, an autonomous activity-summary agent inside LionDesign.',
...
function assembleExample(templateHtml, slidesHtml, title) {
  return templateHtml
    ...
    .replace(
      /<title>.*?<\/title>/,
      `<title>${title} | LionDesign Example</title>`,
    );
}
