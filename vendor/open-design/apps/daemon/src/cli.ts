...
function printRootHelp() {
  ...
  od mcp [--daemon-url <url>]
      ...
      running LionDesign daemon. Wire it into a coding agent
      (Claude Code, Cursor, VS Code, Zed, Windsurf) in another repo
      to pull files from a local LionDesign project without
      exporting a zip.
...
Runs Tavily-backed shallow research through the local LionDesign daemon.
  ...
  if (code === 'EPERM' || code === 'ENETUNREACH') {
    console.error(
      'hint: outbound connect was denied by a sandbox. If you launched ' +
        ...
        'network policy. The LionDesign daemon itself is unaffected - it can be ' +
...
tool calls to a running LionDesign daemon. Wire it into a coding agent
in another repo so the agent can pull files from a local LionDesign
project without exporting a zip every iteration.

Options:
  --daemon-url <url>   LionDesign daemon HTTP base URL. Resolution
...
Tools exposed:
  list_projects                  list every LionDesign project
...
LionDesign; get_artifact and get_file additionally default to the
...
→ MCP server in the LionDesign app. Read-only by design; the daemon
