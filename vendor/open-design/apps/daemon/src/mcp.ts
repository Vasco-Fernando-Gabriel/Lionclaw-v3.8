...
// (Claude Code, Cursor, Zed) pull files from a local LionDesign
...
const PROJECT_ARG = {
  type: 'string',
  description: 'Project id (UUID) or name substring. Optional; defaults to the active project (expires after ~5 minutes of no LionDesign activity).',
} as const;

const TOOL_DEFS = [
  {
    name: 'list_projects',
    description: 'List every LionDesign project on this daemon.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    annotations: { ...READ_ANNOTATIONS, title: 'List LionDesign projects' },
  },
  {
    name: 'get_active_context',
    description:
      'Project + file the user has open in LionDesign right now. Returns {active:false, hint:"..."} when no project is active so the agent can ask the user to interact with LionDesign (the active context expires ~5 minutes after the last user interaction). Most tools default to this when project is omitted, so you rarely need to call this directly.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entry: {
          type: 'string',
          description:
            "Entry file path relative to project root. Defaults to the active file or project's metadata.entryFile. Active-file fallback expires after ~5 minutes of no LionDesign activity.",
        },
        include: {
          type: 'string',
  ...
  },
  {
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'Get LionDesign project' },
  },
  {
    name: 'get_file',
    description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path: {
          type: 'string',
          description:
            'File path relative to project root, forward slashes. Optional; defaults to the active file when project is also omitted. Active-file fallback expires after ~5 minutes of no LionDesign activity.',
        },
        offset: {
          type: 'number',
  ...
  // MCP tools. Skills are recipes that LionDesign itself uses to
...
export async function runMcpStdio({ daemonUrl }: RunMcpOptions): Promise<void> {
  ...
  const server = new Server(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      ...
      instructions: [
        'LionDesign (OD) is a local-first design workspace. The user typically',
        ...
        '',
        'When extending an LionDesign design in another codebase, pull',
  ...
  server.setRequestHandler(ListResourcesRequestSchema, async () => {
    ...
    const resources = [
      {
        uri: 'od://focus/active',
        name: 'Active LionDesign context',
        description: 'The project/file the user has open in LionDesign right now.',
      ...
      switch (name) {
        ...
        case 'get_active_context': {
          ...
          if (!data || data.active === false) {
            return ok({
              active: false,
              hint: 'LionDesign has no active project right now. The active context expires about 5 minutes after the last user interaction with LionDesign, so the user may need to click into a project (or switch tabs inside one) to wake it up. Alternatively, pass project="<id-or-name>" to other tools to bypass active context entirely.',
            });
          }
          return ok(data);
        }
...
// renamed in the LionDesign UI shows up within a few seconds.
...
// open in LionDesign. Returns the resolved id plus, for echo-back to the
...
async function resolveProjectArg(baseUrl: string, arg: unknown): Promise<{ id: string; resolved: ResolvedProject | null; active: ActiveContext | null }> {
  ...
  if (!active || active.active === false || !active.projectId) {
    throw new Error(
      'project arg omitted and LionDesign has no active project. The active context expires about 5 minutes after the last user interaction with LionDesign - the user may need to click into a project to wake it up. Otherwise pass project="<id-or-name>".',
    );
  }
...
function formatError(err: unknown, daemonUrl: string): string {
  ...
  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND') {
    return `cannot reach the LionDesign daemon at ${daemonUrl}. Is it running? Start it with \`pnpm tools-dev\`.`;
  }
  return msg;
}
