...
- `packages/sidecar-proto`: LionDesign sidecar business protocol. Owns app/mode/source constants, namespace validation, stamp descriptor/fields/flags, IPC message schema, status shapes, error semantics, and default product path constants.
- `packages/sidecar`: generic sidecar runtime primitives. Includes bootstrap, IPC transport, path/runtime resolution, launch env, and JSON runtime file helpers; it must not hard-code LionDesign app keys or IPC business messages.
...
- Do not hard-code LionDesign app/source/mode constants in `sidecar` or `platform`.
