...
interface LmStudioSubPanelProps {
  onComplete: SdkCompleteHandler;
}

type TestStatus = 'idle' | 'testing' | 'ok' | 'fail';

export function LmStudioSubPanel({ onComplete }: LmStudioSubPanelProps) {
  ...
  const handleListModels = async () => {
    ...
    setTestMessage('');
    try {
      const result = await window.lionclaw.ollama.listModels('lmstudio', baseUrl.trim());
      ...
      if (result?.models && Array.isArray(result.models)) {
        ...
        setTestMessage(
          n > 0
            ...
            : 'Servidor acessivel, mas nenhum modelo carregado ainda.',
        );
      } else {
        setTestStatus('fail');
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Base URL */}
      <div>
        <label className="block text-xs text-zinc-500 mb-1.5">URL do servidor LM Studio</label>
        <input
          type="text"
          value={baseUrl}
      ...
      <div>
        ...
        {modelSuggestions.length > 0 ? (
          ...
          <input
            ...
            onChange={(e) => setOrchestratorModel(e.target.value)}
            className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2.5 text-sm text-zinc-100 outline-none focus:border-amber-500/50"
            placeholder="Ex: lmstudio-community/Meta-Llama-3.1-8B-Instruct-GGUF"
            autoComplete="off"
          />
        )}
