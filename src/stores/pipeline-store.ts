...
interface PipelineState {
  ...
  _appendAgentStream: (
    projectId: string,
    agent: string,
    entry: { type: string; content?: string; tool?: string },
  ) => void;
    ...
    (set, get) => ({
      ...
      _setProjectState: (projectId: string, patch: Partial<PerProjectState>) => {
        set((state) => {
          ...
          if (projectId === state.activeProjectId) {
            return { projectStates: newMap, _lastTouchedAt: newTouched, ...patch } as Partial<PipelineState> & {
              projectStates: Map<string, PerProjectState>;
              _lastTouchedAt: Map<string, number>;
            };
          }
        ...
        try {
          ...
          if ('error' in result) {
            ...
            useErrorToastStore
              ...
              .pushError(
                { error: result.error },
                { title: 'Falha ao carregar agentes de auditoria', source: 'pipeline' },
              );
            return;
          }
        ...
        try {
          ...
          set((state) => {
            ...
            return {
              ...
              ...(state.activeProjectId === projectId ? { activeProjectId: null, ...initialRuntimeState } : {}),
            };
          });
        } catch (err) {
          const message = err instanceof Error ? err.message : String(err);
      ...
      sendMessage: async (message: string, attachments?: ChatAttachment[]) => {
        ...
        try {
          ...
          if (
            'ok' in result &&
            typeof result.laneBadge === 'number' &&
            result.resumedInSessionId !== useChatStore.getState().currentSessionId
          ) {
            useErrorToastStore.getState().pushNotice(`drive retomado na Lane ${result.laneBadge}`, {
              tone: 'info',
              body: 'O pipeline voltou a ser conduzido nesta lane.',
              source: 'pipeline',
            });
          }
          if ('error' in result && result.code === 'session_not_active') {
            useErrorToastStore.getState().pushError(result, {
              title: laneErrorTitle(result.code, 'Nao foi possivel enviar'),
              source: 'pipeline',
            });
          }
          if ('error' in result) {
      ...
      resetPhase: async (projectId: string, phase: number) => {
        try {
          const result = (await window.lionclaw.pipeline.resetPhase(projectId, phase)) as {
            ok: boolean;
            error?: string;
          };
          if (result.ok) {
      ...
      resetSprint: async (projectId: string, sprintIndex: number) => {
        try {
          const result = (await window.lionclaw.pipeline.resetSprint(projectId, sprintIndex)) as {
            ok: boolean;
            error?: string;
          };
          if (result.ok) {
      ...
      loadSprintHistory: async (projectId: string, sprintIndex: number) => {
        ...
        try {
          const result = (await window.lionclaw.pipeline.getSprintHistory(projectId, sprintIndex)) as
      ...
      _applyStreamToolResult: (projectId, tool, result, isError, toolCallId) => {
        const cur = get().projectStates.get(projectId) ?? get()._createEmptyProjectState();
        let index = toolCallId ? cur.currentToolCalls.findIndex((call) => call.toolCallId === toolCallId) : -1;
      ...
      _handlePhaseChanged: (event: PipelinePhaseChangedEvent) => {
        ...
        get()._setProjectState(event.projectId, {
          ...
          coderStream:
            isPhaseTransition && isCoderPhaseForProject(phaseEventPipelineType, event.phase) ? [] : cur.coderStream,
          evaluatorStream:
            isPhaseTransition && isCoderPhaseForProject(phaseEventPipelineType, event.phase) ? [] : cur.evaluatorStream,
          currentRound: isPhaseTransition ? 0 : cur.currentRound,
          currentAgent: isPhaseTransition ? '' : cur.currentAgent,
          phaseMessages:
            isPhaseTransition && event.phase !== null && !(event.phase in cur.phaseMessages)
        ...
        if (event.phase === null) {
          ...
          set((state) => ({
            projects: state.projects.map((p) =>
              p.id === event.projectId ? { ...p, status: terminalStatus, currentPhase: null } : p,
            ),
          }));
        }
      ...
      init: () => {
        ...
        const unsubStream = window.lionclaw.pipeline.onStream((chunk: PipelineStreamChunk) => {
          ...
          switch (chunk.type) {
            ...
            case 'tool_result': {
              store._applyStreamToolResult(eventProjectId, chunk.tool, chunk.content, chunk.isError, chunk.toolCallId);
              break;
            }
            case 'done':
        ...
        const unsubProjectUpdated = window.lionclaw.pipeline.onProjectUpdated((event: PipelineProjectUpdatedEvent) => {
          const eventProjectId = event.projectId ?? null;
          if (!eventProjectId) {
        ...
        const unsubMessagesUpdated = window.lionclaw.pipeline.onMessagesUpdated(
          (data: { projectId: string; phase: number }) => {
            ...
            const isRelevant = state.activeProjectId === eventProjectId || state.projectStates.has(eventProjectId);
        ...
        const unsubAgentCompleted = window.lionclaw.pipeline.onAgentCompleted((data: { projectId: string }) => {
          const eventProjectId = data.projectId ?? null;
          if (!eventProjectId) {
        ...
        const unsubManifest = window.lionclaw.pipeline.onManifest((data: { projectId: string; manifest: unknown }) => {
          const eventProjectId = data.projectId ?? null;
          if (!eventProjectId) {
        ...
        const unsubAuditProgress = window.lionclaw.pipeline.onAuditAgentProgress(
          (event: PipelineAuditAgentProgressEvent) => {
            ...
            const updated: AuditAgentState = {
              ...
              completedAt: event.status === 'completed' || event.status === 'failed' ? now : existing?.completedAt,
            };
